...
class NeighborConnectionDecoder(nn.Module):
    def __init__(self, channel):
        ...
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)  # ✅ CORRECT
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        S_g_pred = F.interpolate(S_g, scale_factor=8, mode='bilinear', align_corners=False)  # ✅ FIXED
        
        # ---- reverse stage 5 ----
        guidance_g = F.interpolate(S_g, scale_factor=0.25, mode='bilinear', align_corners=False)  # ✅ FIXED
        ra4_feat = self.RS5(x4_rfb, guidance_g)
        S_5 = ra4_feat + guidance_g
        S_5_pred = F.interpolate(S_5, scale_factor=32, mode='bilinear', align_corners=False)  # ✅ FIXED
        
        # ---- reverse stage 4 ----
        guidance_5 = F.interpolate(S_5, scale_factor=2, mode='bilinear', align_corners=False)  # ✅ FIXED
        ra3_feat = self.RS4(x3_rfb, guidance_5)
        S_4 = ra3_feat + guidance_5
        S_4_pred = F.interpolate(S_4, scale_factor=16, mode='bilinear', align_corners=False)  # ✅ FIXED
        
        # ---- reverse stage 3 ----
        guidance_4 = F.interpolate(S_4, scale_factor=2, mode='bilinear', align_corners=False)  # ✅ FIXED
        ra2_feat = self.RS3(x2_rfb, guidance_4)
        S_3 = ra2_feat + guidance_4
        S_3_pred = F.interpolate(S_3, scale_factor=8, mode='bilinear', align_corners=False)  # ✅ FIXED
